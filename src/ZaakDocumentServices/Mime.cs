...
{
    internal class MimeTypeMap
    {
        //from https://raw.githubusercontent.com/samuelneff/MimeTypeMap/master/src/MimeTypes/MimeTypeMap.cs
        ...
        public static string GetMimeType(string extension)
        {
            ...
            string mime;
            return _mappings.Value.TryGetValue(extension, out mime) ? mime : null;
        }
    ...
    public class Mime
    {
        ...
        public static string GetMime(string filename, byte[] data)
        {
            var info = new System.IO.FileInfo(filename);
            if(info.Extension.Length == 0)
            {
                // op basis van de bytes
                return GetMime(data);
            }
            // op basis van mooie lijst
            var mime = MimeTypeMap.GetMimeType(info.Extension);
            if (mime != null)
            {
                return mime;
            }
        ...
        internal static string GetMime(byte[] data)
        {
            // eerst maar eens kijken of het een "simpele" afbeelding is
            {
                var tiff = new byte[]
                {
                    73,
                    73,
                    42
                }; // TIFF Intel byte ordering
                var tiff2 = new byte[]
                {
                    77,
                    77,
                    42
                }; // TIFF Motorola byte ordering
            ...
            // dan maar eens op basis van internet explorer filetype gok logica
            try
            {
                uint mimeType;
        ...
        internal static string GetMime(string filename)
        {
